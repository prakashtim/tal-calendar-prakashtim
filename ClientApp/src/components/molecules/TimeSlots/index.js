...
class TimeSlots extends React.Component {
    constructor() {
        ...
        getAllBookings().then(bookings => {
            ...
            for(let i =0; i< bookings.length; i++){
                this.state.prebookedSlots.push(this.ExtractDateTimeSlot(bookings[i].start))
            }
            this.setState({bookingSlots: this.SetBookingSlotsAsUnavailable(this.state.bookingSlots, this.state.prebookedSlots)})
        })
    }
    ...
    ExtractDateTimeSlot(dateTimeData){
        let slotTime = new Date(dateTimeData)
        let hrs = slotTime.getHours()
        let mins = slotTime.getMinutes()
        if(mins<10)
            mins = '0' + mins
